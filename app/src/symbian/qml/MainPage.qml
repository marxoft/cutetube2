...
MyPage {
    ...
    title: (Settings.currentService) && (serviceModel.count) ? serviceModel.data(serviceModel.match("value", Settings.currentService), "name")
                                                             : "cuteTube2"
    tools: ToolBarLayout {
                ...
                onClicked: {
                    if (!value) {
                        ...
                        appWindow.pageStack.push(Qt.resolvedUrl("plugins/PluginCategoriesPage.qml"),
                                                 {title: value.label}).model.list(value.id);
                    }

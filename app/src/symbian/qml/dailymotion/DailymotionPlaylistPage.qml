...
MyPage {
    ...
    function load(playlistOrId) {
        ...
        videosTab.model.list("/playlists/" + (playlist.id ? playlist.id : playlistOrId), {limit: 50});
    }
    ...
    showProgressIndicator: (playlist.status == QDailymotion.ResourcesRequest.Loading)
                           ...
                           || (videosTab.model.status == QDailymotion.ResourcesRequest.Loading)
    ...
    Connections {
        ...
        onStatusChanged: {
            if ((videosTab.model.status == QDailymotion.ResourcesRequest.Ready) && (videosTab.model.canFetchMore)) {

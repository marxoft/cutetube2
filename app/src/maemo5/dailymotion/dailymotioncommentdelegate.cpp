...
DailymotionCommentDelegate::DailymotionCommentDelegate(ImageCache *cache, QObject *parent) :
...
bool DailymotionCommentDelegate::editorEvent(QEvent *event, QAbstractItemModel *, const QStyleOptionViewItem &option,
...
void DailymotionCommentDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
                                     const QModelIndex &index) const {
    
    qDrawBorderPixmap(painter, option.rect, QMargins(30, 60, 30, 30),
                      QPixmap("/etc/hildon/theme/images/ContactsAppletBubble.png"));
    
    QUrl imageUrl = index.data(DailymotionCommentModel::ThumbnailUrlRole).toUrl();
    ...
    painter->drawText(textRect, Qt::AlignVCenter | Qt::TextWordWrap,
                      QString(tr("by %1 on %2").arg(index.data(DailymotionCommentModel::UsernameRole).toString())
                                               .arg(index.data(DailymotionCommentModel::DateRole).toString())));
    ...
    painter->drawText(textRect, Qt::TextWordWrap, index.data(DailymotionCommentModel::BodyRole).toString());
    painter->restore();
}

QSize DailymotionCommentDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const {
    return QSize(option.rect.width(), option.fontMetrics.boundingRect(option.rect, Qt::TextWordWrap,
                 index.data(DailymotionCommentModel::BodyRole).toString()).height() + 80);
}

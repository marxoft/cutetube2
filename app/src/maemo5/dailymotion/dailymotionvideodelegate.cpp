...
DailymotionVideoDelegate::DailymotionVideoDelegate(ImageCache *cache, QObject *parent) :
...
bool DailymotionVideoDelegate::editorEvent(QEvent *event, QAbstractItemModel *, const QStyleOptionViewItem &option,
...
void DailymotionVideoDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
    ...
    QUrl imageUrl = index.data(DailymotionVideoModel::ThumbnailUrlRole).toUrl();
    ...
    QString duration = index.data(DailymotionVideoModel::DurationRole).toString();
    QFont font;
    ...
    QString text = index.data(DailymotionVideoModel::TitleRole).toString();
    ...
    if (gridMode()) {
        ...
        text.append(QString("\n%1 %2 %3 %4").arg(tr("by")).arg(index.data(DailymotionVideoModel::UsernameRole).toString())
                                            .arg(tr("on")).arg(index.data(DailymotionVideoModel::DateRole).toString()));
...
QSize DailymotionVideoDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &) const {
    return gridMode() ? QSize(124, 132) : QSize(option.rect.width(), 91);
}

bool DailymotionVideoDelegate::gridMode() const {
    return m_gridMode;
}

void DailymotionVideoDelegate::setGridMode(bool enabled) {
    m_gridMode = enabled;
}

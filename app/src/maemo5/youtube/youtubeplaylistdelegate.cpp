...
YouTubePlaylistDelegate::YouTubePlaylistDelegate(ImageCache *cache, QObject *parent) :
...
void YouTubePlaylistDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
    ...
    QUrl imageUrl = index.data(YouTubePlaylistModel::ThumbnailUrlRole).toUrl();
    ...
    const int count = index.data(YouTubePlaylistModel::VideoCountRole).toInt();
    ...
    QString text = index.data(YouTubePlaylistModel::TitleRole).toString();
    ...
    if (gridMode()) {
        ...
        text.append(QString("\n%1 %2 %3 %4").arg(tr("by")).arg(index.data(YouTubePlaylistModel::UsernameRole).toString())
                                            .arg(tr("on")).arg(index.data(YouTubePlaylistModel::DateRole).toString()));
...
QSize YouTubePlaylistDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &) const {
    return gridMode() ? QSize(124, 132) : QSize(option.rect.width(), 91);
}

bool YouTubePlaylistDelegate::gridMode() const {
    return m_gridMode;
}

void YouTubePlaylistDelegate::setGridMode(bool enabled) {
    m_gridMode = enabled;
}

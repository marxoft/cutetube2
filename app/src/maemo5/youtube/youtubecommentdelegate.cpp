...
YouTubeCommentDelegate::YouTubeCommentDelegate(ImageCache *cache, QObject *parent) :
...
bool YouTubeCommentDelegate::editorEvent(QEvent *event, QAbstractItemModel *, const QStyleOptionViewItem &option,
...
void YouTubeCommentDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
                                   const QModelIndex &index) const {
    
    qDrawBorderPixmap(painter, option.rect, QMargins(30, 60, 30, 30),
                      QPixmap("/etc/hildon/theme/images/ContactsAppletBubble.png"));
    
    QUrl imageUrl = index.data(YouTubeCommentModel::ThumbnailUrlRole).toUrl();
    ...
    painter->drawText(textRect, Qt::AlignVCenter | Qt::TextWordWrap,
                      QString(tr("by %1 on %2").arg(index.data(YouTubeCommentModel::UsernameRole).toString())
                                               .arg(index.data(YouTubeCommentModel::DateRole).toString())));
    ...
    painter->drawText(textRect, Qt::TextWordWrap, index.data(YouTubeCommentModel::BodyRole).toString());
    painter->restore();
}

QSize YouTubeCommentDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const {
    return QSize(option.rect.width(), option.fontMetrics.boundingRect(QRect(option.rect.left() + 72,
                                                                            option.rect.top() + 64,
                                                                            option.rect.width() - 40,
                                                                            1000),
                 Qt::TextWordWrap, index.data(YouTubeCommentModel::BodyRole).toString()).height() + 80);
}

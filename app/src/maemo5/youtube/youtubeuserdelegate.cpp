...
YouTubeUserDelegate::YouTubeUserDelegate(ImageCache *cache, QObject *parent) :
...
void YouTubeUserDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
    ...
    QUrl imageUrl = index.data(YouTubeUserModel::ThumbnailUrlRole).toUrl();
    ...
    QString text = index.data(YouTubeUserModel::UsernameRole).toString();
    ...
    if (gridMode()) {
        ...
        const qint64 count = index.data(YouTubeUserModel::SubscriberCountRole).toLongLong();
...
QSize YouTubeUserDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &) const {
    return gridMode() ? QSize(93, 116) : QSize(option.rect.width(), 91);
}

bool YouTubeUserDelegate::gridMode() const {
    return m_gridMode;
}

void YouTubeUserDelegate::setGridMode(bool enabled) {
    m_gridMode = enabled;
}

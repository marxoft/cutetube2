...
PluginPlaylistDelegate::PluginPlaylistDelegate(ImageCache *cache, QObject *parent) :
...
void PluginPlaylistDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
    ...
    QUrl imageUrl = index.data(PluginPlaylistModel::ThumbnailUrlRole).toUrl();
    ...
    const int count = index.data(PluginPlaylistModel::VideoCountRole).toInt();
    ...
    QString text = index.data(PluginPlaylistModel::TitleRole).toString();
...
QSize PluginPlaylistDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &) const {
    return gridMode() ? QSize(124, 132) : QSize(option.rect.width(), 91);
}

bool PluginPlaylistDelegate::gridMode() const {
    return m_gridMode;
}

void PluginPlaylistDelegate::setGridMode(bool enabled) {
    m_gridMode = enabled;
}

...
PluginPlaybackDialog::PluginPlaybackDialog(const QString &service, QWidget *parent) :
    Dialog(parent),
...
QString PluginPlaybackDialog::streamUrl() const {
    return m_streamSelector->currentValue().toMap().value("url").toString();
}

void PluginPlaybackDialog::accept() {
    Settings::setDefaultPlaybackFormat(m_model->service(), m_streamSelector->valueText());
    Dialog::accept();
}

void PluginPlaybackDialog::list(const QString &videoId) {
    ...
    case ResourcesRequest::Ready:
        if (m_model->rowCount() > 0) {
            m_streamSelector->setCurrentIndex(qMax(0, m_model->match("name",
                                                   Settings::defaultPlaybackFormat(m_model->service()))));
        }
        else {

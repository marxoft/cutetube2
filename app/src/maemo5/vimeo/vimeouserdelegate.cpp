...
VimeoUserDelegate::VimeoUserDelegate(ImageCache *cache, QObject *parent) :
...
void VimeoUserDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const {
    painter->save();
    ...
    QUrl imageUrl = index.data(VimeoUserModel::ThumbnailUrlRole).toUrl();
    ...
    QString text = index.data(VimeoUserModel::UsernameRole).toString();
    ...
    if (gridMode()) {
        ...
        const qint64 count = index.data(VimeoUserModel::SubscriberCountRole).toLongLong();
...
QSize VimeoUserDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &) const {
    return gridMode() ? QSize(93, 116) : QSize(option.rect.width(), 91);
}

bool VimeoUserDelegate::gridMode() const {
    return m_gridMode;
}

void VimeoUserDelegate::setGridMode(bool enabled) {
    m_gridMode = enabled;
}

...
QString Settings::activeColorString() {
    return value("Appearance/activeColorString", QString("color7")).toString();
}

void Settings::setActiveColorString(const QString &color) {
    if (color != activeColorString()) {
        setValue("Appearance/activeColorString", color);

        if (self) {
            emit self->activeColorStringChanged(color);
        }
    }
}

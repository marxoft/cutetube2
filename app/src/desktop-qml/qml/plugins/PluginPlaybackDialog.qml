...
MyDialog {
    ...
    content: ComboBox {
        ...
        model: PluginStreamModel {
            ...
            onStatusChanged: {
                ...
                case ResourcesRequest.Ready:
                    if (count > 0) {
                        streamSelector.currentIndex = Math.max(0, match("name", Settings.defaultPlaybackFormat(service)));
                    }
                    else {
        ...
        textRole: "name"
        onActivated: Settings.setDefaultPlaybackFormat(streamModel.service, streamModel.data(index, "name"))
    }
    buttons: [
        Button {
            text: qsTr("&Cancel")
